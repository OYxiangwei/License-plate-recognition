...
public class SVMTrain {
	...
	private void learn2Plate(float bound, final String name) {
		...
		for (int i = 0; i < boundry; i++) {
			...
			String str = "res/train/data/plate_detect_svm/train/" + name + "/" + name + "_"
		...
		for (int i = boundry; i < size; i++) {
			System.out.println(files.get(i));
			Mat img = imread(files.get(i));
			String str = "res/train/data/plate_detect_svm/test/" + name + "/" + name + "_"
					+ Integer.valueOf(i).toString() + ".jpg";
			imwrite(str, img);
		}
	}
	...
	public int svmTrain(boolean dividePrepared, boolean trainPrepared) {
		...
		if (!trainPrepared && !classes.empty() && !trainingData.empty()) {
			CvSVMParams SVM_params = new CvSVMParams(CvSVM.C_SVC, CvSVM.RBF, 0.1, 1, 0.1, 1, 0.1, 0.1, new CvMat(),
					new CvTermCriteria().type(CV_TERMCRIT_ITER).max_iter(100000).epsilon(0.0001));
